...
function breedList(con){
  var container = document.getElementById(con);
  var breedList = document.createElement("select");
  for (var j = 0; j < breeds.length; j++) {
        var b = document.createElement("option");
        b.value = breeds[j];
        b.text = breeds[j];
        breedList.appendChild(b);
  }
  container.appendChild(breedList);
}

function charList(con){
  var container = document.getElementById(con);
  var charList = document.createElement("select");
  for (var j = 0; j < characteristicList.length; j++) {
        var c = document.createElement("option");
        c.value = characteristicList[j];
        c.text = characteristicList[j];
        charList.appendChild(c);
  }
  container.appendChild(charList);
}

function opacList(con){
  var container = document.getElementById(con);
  var opacList = document.createElement("select");
  for (var j = 0; j < opacityList.length; j++) {
        var o = document.createElement("option");
        o.value = opacityList[j];
        o.text = opacityList[j];
        opacList.appendChild(o);
  }
  container.appendChild(opacList);
}